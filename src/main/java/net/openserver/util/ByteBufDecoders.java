...
public class ByteBufDecoders {
    ...
    public static int getPacketSize(int opcode, ByteBuf packet) {
        ByteBuf opcodeBuffer = Unpooled.buffer(5);
        ByteBufDecoders.writeVarInt(opcodeBuffer, opcode);
        return opcodeBuffer.readableBytes() + packet.readableBytes();
    }

    public static ByteBuf writeHeader(int opcode, ByteBuf packet) {
        ByteBuf message = Unpooled.buffer();
        int packetSize = getPacketSize(opcode, packet);
        ByteBufDecoders.writeVarInt(message, packetSize);
        ByteBufDecoders.writeVarInt(message, opcode);
        message.writeBytes(packet.array());
        return message;
    }
}
